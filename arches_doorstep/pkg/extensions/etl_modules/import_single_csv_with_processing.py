# Based on Arches ImportSingleCsv
# Copyright AGPL-3.0-only, Arches Project, 2024-
# Changes copyright AGPL-3.0-only, Flax &amp; Teal Limited, 2024-
...
details = {
    ...
    "etl_type": "import",
    "component": "views/components/etl_modules/import_single_csv_with_processing",
    "componentname": "import_single_csv_with_processing",
    "modulename": "import_single_csv_with_processing.py",
    "classname": "ImportSingleCsvWithProcessing",
    "config": {"bgColor": "#f5c60a", "circleColor": "#f9dd6c"},
    "icon": "fa fa-upload",
    "slug": "import-single-csv-with-processing",
}

class ImportSingleCsvWithProcessing(BaseImportModule):
    ...
    def _data_info_in_process(self, csv_file, only=None, data=None):
        ...
        if not doorstep_ini:
            raise RuntimeError("You have attempted to run validation without setting up any processors in ARCHES_DOORSTEP_INI")
    ...
    def process(self, request):
        ...
        data = self.data_info(csvFile.read(), ["date_checker", "concept_checker", "resource_checker"] , data_info)
